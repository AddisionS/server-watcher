...
def evaluate_and_log_alert(
    *,
    ...
    if (
        ...
        logger.info(
            ...
            extra={"temperature": temperature,
                   "min": threshold_cache.THRESHOLDS["temp_min"],
                   "max": threshold_cache.THRESHOLDS["temp_max"]}
        )
    ...
    if (
        ...
        logger.info(
            ...
            extra={"humidity": humidity,
                   "min": threshold_cache.THRESHOLDS["humidity_min"],
                   "max": threshold_cache.THRESHOLDS["humidity_max"]}
        )

    if not breached:
        logger.info(
            "Values within threshold, no alert sent",
            extra={"temperature": temperature, "humidity": humidity}
        )
        return                         
    ...
    if device_name is None:
        logger.warning("Device not found in database", extra={"device_id": device_id})
        device_name = "unknown"
